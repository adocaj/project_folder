...
public class Board implements ActionListener, Game {
    ...
    UserInteraction tacToe; // take the tic tac toe object created in the class of the same name
    //**********************************************************/
    ...
    Board(int frameSize) {
        ...
        mSearch = (MonteCarloSearch) gameFactory.getGameType(ObjectChoice.mSearchObject);
    }
    //**********************************************************/
    ...
    public void setButtons() {
        ...
        for (int i = 0; i < frameSize; i++) {
            for (int j = 0; j < frameSize; j++) {
                ...
                frameButtons[i][j].setText(Game.Empty); // initially the squares are set to an empty string
                frameButtons[i][j].addActionListener(this); // enables association of clicks to the class object, in our case UserInteraction
    ...
    public void compMovesSecond(){
        if (compPlayStatus && !compMovesFirst) {
            mSearch.setCompMarker(Game.O);
            mSearch.setPlayerMarker(Game.X);
        }
    }
    //**********************************************************/
    ...
    public void makeCompFirstMove(){
        ...
        if (compPlayStatus && compMovesFirst) {
            ...
            mSearch.setCompMarker(Game.X); // the computer will play as "X"
            mSearch.setPlayerMarker(Game.O);
    ...
    public void actionPerformed(ActionEvent event) {   
        ...
        if(!buttonClicked.getText().equals(Game.Empty)) // make sure clicked buttons don't change if clicked again
            return;

        // clicking alternating buttons, produces alternating X and O
        marker = this.alternatingNum % 2 == 0 ? Game.X : Game.O;
        ...
        if (state.isPresentMoveTerminal()) { // if the present move is a game ending move
            ...
            if(playAgain == JOptionPane.YES_OPTION) { // if the user wants to play again
                ...
                UserInteraction tacToe = (UserInteraction) gameFactory.getGameType(ObjectChoice.userInteractionObject);
                tacToe.buildFrame(); // and build a new game frame for the user to choose from                
    ...
    public void applyComputerChoice(int row, int col, String marker) {
        ...
        if (state.isPresentMoveTerminal()) { // if the compute made move is terminal
            ...
            if(playAgain == JOptionPane.YES_OPTION) { // if the user wants to play again
                ...
                frame.dispose();   // dispose the buttons frame                
                UserInteraction tacToe = (UserInteraction) gameFactory.getGameType(ObjectChoice.userInteractionObject);            
                tacToe.buildFrame(); // and build a new interactive frame             
            } else {
                System.exit(0); // otherwise we exit
            }                  
        } 
    ...
    public void clearButtons(){
        for (int i = 0; i < fButtonsList.size(); i++) {
            fButtonsList.get(i).setText(Game.Empty);
        }
